    ...
    <footer>
      <section className={`bg-black flex justify-center ${styles.footerTop}`}>
        <div className={`flex`}>
          <div className={`${styles.footerLeft}`}>
            ...
            <p className={`text-white mb-0`}>Take the first step to run your workspace business with the leading space management software today.</p>
          </div>
      ...
      <section className={`flex ${styles.footerBottom}`}>
        ...
        <div className={`lg:grow ${styles.footerBottomMiddle}`}>
          <div className={`flex`}>
            {FooterListData.map((item, index1) => {
              return (
                <div key={index1} className={`${styles.footerBottomMiddleItem}`}>
                  <h3 className={`mb-0`}>{item.title}</h3>
                  {item.children.map((temp, index2) => (
                    <p className={`mb-0`} key={index2}>{temp}</p>
                  ))}
                </div>
              )
            })}
        ...
        <div className={`${styles.footerBottomRight}`}>
          ...
          <p className={`mb-0`}>Stay up to date with coworking insights, feature highlights, company news,  upcoming webinars, and eBooks.</p>
          <div className={`${styles.footerBottomRightInputBox}`}>
            <input type="text" className={``} placeholder="Enter your business email" />
            <button
